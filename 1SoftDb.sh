...
# 宏基因组软件和数据库安装 Metagenomic software & database
    ...
    # 测试环境为Linux Ubuntu 16.04+ / CentOS 7+
    # 版本: 1.11, 2021/5/7
...
### 软件管理器Conda
    ...
    # 不要忘记执行这一句，不然你就用重启terminal了
    source ~/.bashrc
    
    # 查看版本，conda 4.8.3, python 2.7.19
    conda -V 
    python --version
    cd ..
...
[一文掌握Conda软件安装：虚拟环境、软件通道、加速solving、跨服务器迁移](https://mp.weixin.qq.com/s/2qJpSqonSWZQBIeN7E1xUA)
...
表格统计工具csvtk和序列处理seqkit

    # conda安装，可能有点旧
    # conda install csvtk -c bioconda -y
    conda install seqkit -c bioconda -y 
    # 直接下载最新版 https://github.com/shenwei356，如以csvtk为例手动安装
    cd ~/db
...
## conda导入打包好的Qiime2环境
    ...
    # 下载已打包好的Qiime2环境
    wget -c http://210.75.224.110/db/conda/qiime2-2021.2.tar.gz

    # 指定安装目录
    mkdir -p ${soft}/envs/qiime2
    tar -xvzf qiime2-2021.2.tar.gz -C ${soft}/envs/qiime2
    # 方法1. 启动/退出环境
    source ${soft}/envs/qiime2/bin/activate
    ...
    # 推出环境
    source ${soft}/envs/qiime2/bin/deactivate


## 安装 conda-pack 打包已有环境

准备工作，安装conda-pack提供打包功能 
...
下载安装包并解压安装：软件包可从以下网址、或上课微信群索引百度云链接

    # 指定环境名称，如metagenome_env, meta,  metawrap1.3, humann2, humann3, qiime2-2021.2
    n=如metagenome_env
    # 下载
...
安装数据库(注：数据库下载慢或失败，附录有国内备份链接)
    ...
    # metaphlan2数据库默认位于${soft}/envs/biobakery/bin/ 程序所在目录的db_v20 (v0.11)和databases(2.8.1)下
    humann2_config --print
    # 例如，链接下载的metaphlan2：
    ln -s `pwd`/metaphlan2 ${soft}/envs/biobakery/bin/databases
...
软件安装
    ...
    # 激活环境
    source ${soft}/envs/rgi/bin/activate
...
### metawrap分箱和提纯
    ...
    # 配置文件通常为${soft}/envs/metawrap/bin/config-metawrap
...
常见问题：Perl版本不对，人工指定perl版本如下
    ...
    PERL5LIB=${soft}/envs/kraken2/lib/site_perl/5.26.2/x86_64-linux-thread-multi:${soft}/envs/kraken2/lib/site_perl/5.26.2:${soft}/envs/kraken2/lib/5.26.2/x86_64-linux-thread-multi:${soft}/envs/kraken2/lib/5.26.2
