...
if ("pdf" == "png") {
    png(filename="metaphlan4/taxonomy_high.tsv.YA04_YA23_EB01_EB02_P01.vennDiagram.png", width=4, height=4,
        res=300, units="cm")
} else if ("pdf" == "eps") {
    postscript(file="metaphlan4/taxonomy_high.tsv.YA04_YA23_EB01_EB02_P01.vennDiagram.eps", onefile=FALSE, horizontal=FALSE,
               paper="special", width=10, height = 12, pointsize=10)
} else if ("pdf" == "pdf") {
    pdf(file="metaphlan4/taxonomy_high.tsv.YA04_YA23_EB01_EB02_P01.vennDiagram.pdf", width=4, height=4)
}else if ("pdf" == "svg") {
    svg(filename="metaphlan4/taxonomy_high.tsv.YA04_YA23_EB01_EB02_P01.vennDiagram.svg", width=4, height=4,
        pointsize=10)
} else {
...
if (! FALSE) {
    ...
    if("YA04" != "CHENTONG"){
        YA04 <- data[grepl("\\<YA04\\>",data[,2]),1]
        num <- num + 1
    }

    if("YA23" != "CHENTONG"){
        YA23 <- data[grepl("\\<YA23\\>",data[,2]),1]
        num <- num + 1
    }

    if("EB01" != "CHENTONG"){
        EB01 <- data[grepl("\\<EB01\\>",data[,2]),1]
        num <- num + 1
    }

    if("EB02" != "CHENTONG"){
        EB02 <- data[grepl("\\<EB02\\>",data[,2]),1]
        num <- num + 1
    }

    if("P01" != "CHENTONG"){
        P01 <- data[grepl("\\<P01\\>",data[,2]),1]
        num <- num + 1
    }
    ...
    if(num == 5){
        ...
        p <- venn.diagram(
            x = list(YA04=YA04, EB02=EB02, YA23=YA23,
                     EB01=EB01),
            filename = NULL, col = "black", lwd = 1,
            fill = color_v,
            alpha = 0.50, main="",
        ...
        p <- venn.diagram(
            x = list(YA04=YA04, YA23=YA23, EB01=EB01),
