...
if ("pdf" == "png") {
    png(filename="metaphlan4/group_high.tsv.Y_E_C.vennDiagram.png", width=4, height=4,
        res=300, units="cm")
} else if ("pdf" == "eps") {
    postscript(file="metaphlan4/group_high.tsv.Y_E_C.vennDiagram.eps", onefile=FALSE, horizontal=FALSE,
               paper="special", width=10, height = 12, pointsize=10)
} else if ("pdf" == "pdf") {
    pdf(file="metaphlan4/group_high.tsv.Y_E_C.vennDiagram.pdf", width=4, height=4)
}else if ("pdf" == "svg") {
    svg(filename="metaphlan4/group_high.tsv.Y_E_C.vennDiagram.svg", width=4, height=4,
        pointsize=10)
} else {
...
if (! FALSE) {
    ...
    if(num == 5){
        ...
        p <- venn.diagram(
            x = list(Y=Y, CHENTONG=CHENTONG, E=E,
                     C=C),
            filename = NULL, col = "black", lwd = 1,
            fill = color_v,
            alpha = 0.50, main="",
