...
if ("pdf" == "png") {
    png(filename="metaphlan4/group_high.tsv.Cancer_Healthy_Early.vennDiagram.png", width=4, height=4,
        res=300, units="cm")
} else if ("pdf" == "eps") {
    postscript(file="metaphlan4/group_high.tsv.Cancer_Healthy_Early.vennDiagram.eps", onefile=FALSE, horizontal=FALSE,
               paper="special", width=10, height = 12, pointsize=10)
} else if ("pdf" == "pdf") {
    pdf(file="metaphlan4/group_high.tsv.Cancer_Healthy_Early.vennDiagram.pdf", width=4, height=4)
}else if ("pdf" == "svg") {
    svg(filename="metaphlan4/group_high.tsv.Cancer_Healthy_Early.vennDiagram.svg", width=4, height=4,
        pointsize=10)
} else {
...
if (! FALSE) {
    ...
    data <- read.table(file="metaphlan4/group_high.tsv", sep="\t", quote="")

    num <- 0

    if("Cancer" != "CHENTONG"){
        Cancer <- data[grepl("\\<Cancer\\>",data[,2]),1]
        num <- num + 1
    }

    if("Healthy" != "CHENTONG"){
        Healthy <- data[grepl("\\<Healthy\\>",data[,2]),1]
        num <- num + 1
    }

    if("Early" != "CHENTONG"){
        Early <- data[grepl("\\<Early\\>",data[,2]),1]
        num <- num + 1
    }
    ...
    if(num == 5){
        p <- venn.diagram(
            x = list(Cancer=Cancer, CHENTONG=CHENTONG,
                     CHENTONG=CHENTONG, Healthy=Healthy,
                     Early=Early),
        ...
        p <- venn.diagram(
            x = list(Cancer=Cancer, CHENTONG=CHENTONG, Healthy=Healthy,
                     Early=Early),
            filename = NULL, col = "black", lwd = 1,
            fill = color_v,
            alpha = 0.50, main="",
        ...
        p <- venn.diagram(
            x = list(Cancer=Cancer, Healthy=Healthy, Early=Early),
        ...
        p <- venn.diagram(
            x = list(Cancer=Cancer, Healthy=Healthy),
            filename = NULL, col = "transparent",
            fill = color_v,
            alpha = 0.50, main="",
            label.col = c("black"),
