...
if ("pdf" == "png") {
    png(filename="metaphlan4/taxonomy_high.tsv.C1_C2_H1_H2_E1.vennDiagram.png", width=4, height=4,
        res=300, units="cm")
} else if ("pdf" == "eps") {
    postscript(file="metaphlan4/taxonomy_high.tsv.C1_C2_H1_H2_E1.vennDiagram.eps", onefile=FALSE, horizontal=FALSE,
               paper="special", width=10, height = 12, pointsize=10)
} else if ("pdf" == "pdf") {
    pdf(file="metaphlan4/taxonomy_high.tsv.C1_C2_H1_H2_E1.vennDiagram.pdf", width=4, height=4)
}else if ("pdf" == "svg") {
    svg(filename="metaphlan4/taxonomy_high.tsv.C1_C2_H1_H2_E1.vennDiagram.svg", width=4, height=4,
        pointsize=10)
} else {
...
if (! FALSE) {
    ...
    if("H1" != "CHENTONG"){
        H1 <- data[grepl("\\<H1\\>",data[,2]),1]
        num <- num + 1
    }

    if("H2" != "CHENTONG"){
        H2 <- data[grepl("\\<H2\\>",data[,2]),1]
        num <- num + 1
    }

    if("E1" != "CHENTONG"){
        E1 <- data[grepl("\\<E1\\>",data[,2]),1]
        num <- num + 1
    }
