...
*   01PPT/
    folder contains the basic knowledge introduction and experience summary of metagenomic analysis.
    文件夹中包含宏基因组分析相关基础知识介绍和经验总结。
*   seq/
    The seq folder contains the fastq compressed files for testing
    seq文件夹包含测试用fastq压缩文件  
*   temp/
    The temp folder contains the intermediate files generated by the EasyMetagenome pipeline
    temp文件夹包含宏基因组分析流程产生的中间文件 
*   result/
    The result folder stores the results of the EasyMetagenome pipeline
    result文件夹保存宏基因组分析流程的结果 
*   result12
    The result12 folder stores result files used to assist visual testing
    result12文件夹保存用于辅助可视化测试的结果文件 
*   binning
