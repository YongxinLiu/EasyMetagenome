...
if ("pdf" == "png") {
    png(filename="metaphlan4/taxonomy_high.tsv.C1_C2_Y1_Y2_Y3.vennDiagram.png", width=4, height=4,
        res=300, units="cm")
} else if ("pdf" == "eps") {
    postscript(file="metaphlan4/taxonomy_high.tsv.C1_C2_Y1_Y2_Y3.vennDiagram.eps", onefile=FALSE, horizontal=FALSE,
               paper="special", width=10, height = 12, pointsize=10)
} else if ("pdf" == "pdf") {
    pdf(file="metaphlan4/taxonomy_high.tsv.C1_C2_Y1_Y2_Y3.vennDiagram.pdf", width=4, height=4)
}else if ("pdf" == "svg") {
    svg(filename="metaphlan4/taxonomy_high.tsv.C1_C2_Y1_Y2_Y3.vennDiagram.svg", width=4, height=4,
        pointsize=10)
} else {
...
if (! FALSE) {
    ...
    if("Y1" != "CHENTONG"){
        Y1 <- data[grepl("\\<Y1\\>",data[,2]),1]
        num <- num + 1
    }

    if("Y2" != "CHENTONG"){
        Y2 <- data[grepl("\\<Y2\\>",data[,2]),1]
        num <- num + 1
    }

    if("Y3" != "CHENTONG"){
        Y3 <- data[grepl("\\<Y3\\>",data[,2]),1]
        num <- num + 1
    }
