...
if (! FALSE) {
    ...
    if("Centenarians" != "CHENTONG"){
        Centenarians <- data[grepl("\\<Centenarians\\>",data[,2]),1]
        num <- num + 1
    }

    if("Young" != "CHENTONG"){
        Young <- data[grepl("\\<Young\\>",data[,2]),1]
        num <- num + 1
    }
    ...
    if(num == 5){
        p <- venn.diagram(
            x = list(Centenarians=Centenarians, CHENTONG=CHENTONG,
                     CHENTONG=CHENTONG, Young=Young,
                     All=All),
        ...
        p <- venn.diagram(
            x = list(Centenarians=Centenarians, CHENTONG=CHENTONG, Young=Young,
                     All=All),
            filename = NULL, col = "black", lwd = 1,
            fill = color_v,
            alpha = 0.50, main="",
        ...
        p <- venn.diagram(
            x = list(Centenarians=Centenarians, Young=Young, All=All),
        ...
        p <- venn.diagram(
            x = list(Centenarians=Centenarians, Young=Young),
            filename = NULL, col = "transparent",
            fill = color_v,
            alpha = 0.50, main="",
            label.col = c("black"),
