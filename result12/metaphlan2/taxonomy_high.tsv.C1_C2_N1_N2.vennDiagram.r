...
if ("pdf" == "png") {
    png(filename="metaphlan2/taxonomy_high.tsv.C1_C2_N1_N2.vennDiagram.png", width=4, height=4,
        res=300, units="cm")
} else if ("pdf" == "eps") {
    postscript(file="metaphlan2/taxonomy_high.tsv.C1_C2_N1_N2.vennDiagram.eps", onefile=FALSE, horizontal=FALSE,
               paper="special", width=10, height = 12, pointsize=10)
} else if ("pdf" == "pdf") {
    pdf(file="metaphlan2/taxonomy_high.tsv.C1_C2_N1_N2.vennDiagram.pdf", width=4, height=4)
}else if ("pdf" == "svg") {
    svg(filename="metaphlan2/taxonomy_high.tsv.C1_C2_N1_N2.vennDiagram.svg", width=4, height=4,
        pointsize=10)
} else {
...
if (! FALSE) {
    ...
    data <- read.table(file="metaphlan2/taxonomy_high.tsv", sep="\t", quote="")
