...
if ("pdf" == "png") {
    png(filename="metaphlan4/group_high.tsv.Cancer_Normal_All.vennDiagram.png", width=4, height=4,
        res=300, units="cm")
} else if ("pdf" == "eps") {
    postscript(file="metaphlan4/group_high.tsv.Cancer_Normal_All.vennDiagram.eps", onefile=FALSE, horizontal=FALSE,
               paper="special", width=10, height = 12, pointsize=10)
} else if ("pdf" == "pdf") {
    pdf(file="metaphlan4/group_high.tsv.Cancer_Normal_All.vennDiagram.pdf", width=4, height=4)
}else if ("pdf" == "svg") {
    svg(filename="metaphlan4/group_high.tsv.Cancer_Normal_All.vennDiagram.svg", width=4, height=4,
        pointsize=10)
} else {
...
if (! FALSE) {
    ...
    if("Normal" != "CHENTONG"){
        Normal <- data[grepl("\\<Normal\\>",data[,2]),1]
        num <- num + 1
    }
    ...
    if(num == 5){
        p <- venn.diagram(
            x = list(Cancer=Cancer, CHENTONG=CHENTONG,
                     CHENTONG=CHENTONG, Normal=Normal,
                     All=All),
        ...
        p <- venn.diagram(
            x = list(Cancer=Cancer, CHENTONG=CHENTONG, Normal=Normal,
                     All=All),
            filename = NULL, col = "black", lwd = 1,
            fill = color_v,
            alpha = 0.50, main="",
        ...
        p <- venn.diagram(
            x = list(Cancer=Cancer, Normal=Normal, All=All),
        ...
        p <- venn.diagram(
            x = list(Cancer=Cancer, Normal=Normal),
            filename = NULL, col = "transparent",
            fill = color_v,
            alpha = 0.50, main="",
            label.col = c("black"),
