...
3.  项目文件夹准备测序数据(seq/\*.fq.gz)和样本元数据(result12/metadata.txt)
...
mkdir -p seq temp result12

元数据

# 上传元数据metadata.txt至result12目录，此处下载并重命名
wget http://www.imeta.science/github/EasyMetagenome/result12/metadata.txt
mv metadata.txt result12/metadata.txt
# 检查文件格式，^I为制表符，$为Linux换行，^M$为Windows回车，^M为Mac换行符
cat -A result12/metadata.txt

# 元数据细节优化
# 转换Windows回车为Linux换行
sed -i 's/\r//' result12/metadata.txt
# 去除数据中的一个多余空格
sed -i 's/Male  /Male/' result12/metadata.txt
cat -A result12/metadata.txt
...
# 基于样本元数据提取样本列表命令解析
# 去掉表头
tail -n+2 result12/metadata.txt
# 提取第一列样本名
tail -n+2 result12/metadata.txt|cut -f1
# 循环处理样本
for i in `tail -n+2 result12/metadata.txt|cut -f1`;do echo "Processing "$i; done
...
# seq 目录下已经有测试文件，下载跳过
cd seq/
  awk '{system("wget -c http://www.imeta.science/github/EasyMetagenome/seq/"$1"_1.fq.gz")}' <(tail -n+2 ../result12/metadata.txt)
awk '{system("wget -c http://www.imeta.science/github/EasyMetagenome/seq/"$1"_2.fq.gz")}' <(tail -n+2 ../result12/metadata.txt)
cd ..
...
# 整理fastqc报告，输出multiqc_report.html至result12/qc目录
multiqc -d seq/ -o result12/qc -f 

查看右侧result12/qc目录中multiqc\_report.html，单击，选择`View in Web Browser`查看可交互式报告。
...
# 多样本并行
tail -n+2 result12/metadata.txt|cut -f1|rush -j 2 \
...
awk '{system("mv temp/qc/"$1"_1_kneaddata_paired_1.fastq temp/qc/"$1"_1.fastq")}' <(tail -n+2 result12/metadata.txt)
awk '{system("mv temp/qc/"$1"_1_kneaddata_paired_2.fastq temp/qc/"$1"_2.fastq")}' <(tail -n+2 result12/metadata.txt)
ls -l temp/qc/
...
# 筛选重点结果列
cut -f 1,2,4,12,13 temp/kneaddata.txt | sed 's/_1_kneaddata//' > result12/qc/sum.txt
cat result12/qc/sum.txt
# 对齐方式查看表格
csvtk -t pretty result12/qc/sum.txt

# 用R代码统计下质控结果
Rscript -e "data=read.table('result12/qc/sum.txt', header=T, row.names=1, sep='\t'); summary(data)"
# R转换宽表格为长表格
Rscript -e "library(reshape2); data=read.table('result12/qc/sum.txt', header=T,row.names=1, sep='\t'); write.table(melt(data), file='result12/qc/sum_long.txt',sep='\t', quote=F, col.names=T, row.names=F)"
cat result12/qc/sum_long.txt
# 可用 https://www.bic.ac.cn/ImageGP/ 绘图展示

## 1.4 质控后质量评估

整理bowtie2, trimmomatic, fastqc报告，接头和PCR污染率一般小于1%。结果见：result12/qc/multiqc\_report\_1.html

fastqc temp/qc/*.fastq -t 2
multiqc -d temp/qc/ -o result12/qc/
...
mkdir -p temp/concat
# 双端合并为单个文件
for i in `tail -n+2 result12/metadata.txt|cut -f1`;do 
...
*   整合后的输出：
*   result12/metaphlan3/taxonomy.tsv 物种丰度表
*   result12/metaphlan3/taxonomy.spf 物种丰度表（用于stamp分析）
*   result12/humann3/pathabundance\_relab\_unstratified.tsv 通路丰度表
*   result12/humann3/pathabundance\_relab\_stratified.tsv 通路物种组成丰度表
...
tail -n+2 result12/metadata.txt|cut -f1|rush -j 3 \
...
mkdir -p result12/metaphlan4
# 合并、修正样本名、预览
merge_metaphlan_tables.py temp/humann3/*_metaphlan_bugs_list.tsv | \
sed 's/_metaphlan_bugs_list//g' > result12/metaphlan4/taxonomy.tsv
head -n5 result12/metaphlan4/taxonomy.tsv

### 转换为stamp的spf格式

# metaphlan4较2增加更多unclassifed和重复结果，用sort和uniq去除
metaphlan_to_stamp.pl result12/metaphlan4/taxonomy.tsv \
|sort -r | uniq > result12/metaphlan4/taxonomy.spf
head result12/metaphlan4/taxonomy.spf
...
mkdir -p result12/humann3
humann_join_tables --input temp/humann3 \
--file_name pathabundance \
--output result12/humann3/pathabundance.tsv
# 样本名调整：删除列名多余信息
head result12/humann3/pathabundance.tsv
sed -i 's/_Abundance//g' result12/humann3/pathabundance.tsv
# 预览和统计
head result12/humann3/pathabundance.tsv
csvtk -t stat result12/humann3/pathabundance.tsv

标准化为相对丰度relab(1)或百万比cpm(1,000,000)

humann_renorm_table \
--input result12/humann3/pathabundance.tsv \
--units relab \
--output result12/humann3/pathabundance_relab.tsv
head -n5 result12/humann3/pathabundance_relab.tsv

分层结果：功能和对应物种表(stratified)和功能组成表(unstratified)

humann_split_stratified_table \
--input result12/humann3/pathabundance_relab.tsv \
--output result12/humann3/ 
  # 可以使用stamp进行统计分析
...
*   输入数据：通路丰度表格 result12/humann3/pathabundance.tsv
*   输入数据：实验设计信息 result12/metadata.txt
*   中间数据：包含分组信息的通路丰度表格文件 result12/humann3/pathabundance.pcl
*   输出结果：result12/humann3/associate.txt

在通路丰度中添加分组

## 提取样品列表
head -n1 result12/humann3/pathabundance.tsv | sed 's/# Pathway/SampleID/' | tr '\t' '\n' > temp/header
## 对应分组，本示例分组为第2列($2)，根据实际情况修改
awk 'BEGIN{FS=OFS="\t"}NR==FNR{a[$1]=$2}NR>FNR{print a[$1]}' result12/metadata.txt temp/header | tr '\n' '\t'|sed 's/\t$/\n/' > temp/group
# 合成样本、分组+数据
cat <(head -n1 result12/humann3/pathabundance.tsv) temp/group <(tail -n+2 result12/humann3/pathabundance.tsv) \
> result12/humann3/pathabundance.pcl
head -n5 result12/humann3/pathabundance.pcl
tail -n5 result12/humann3/pathabundance.pcl
...
wget -c http://www.imeta.science/github/EasyMetagenome/result12/humann2/hmp_pathabund.pcl
/bin/cp -f hmp_pathabund.pcl result12/humann3/
  # 设置输入文件名
  pcl=result12/humann3/hmp_pathabund.pcl
# 统计表格行、列数量
csvtk -t stat ${pcl}
...
--output result12/humann3/associate.txt
wc -l result12/humann3/associate.txt
head -n5 result12/humann3/associate.txt
...
--output result12/humann3/barplot_${path}.pdf --sort sum metadata 
...
--input temp/humann3/ \
--file_name ko \
--output result12/humann3/ko.tsv
sed -i '1s/_Abundance-RPKs//g' result12/humann3/ko.tsv
tail result12/humann3/ko.tsv
# 与pathabundance类似，可进行标准化renorm、分层stratified、柱状图barplot等操作

KO合并为高层次L2, L1通路代码

wc -l result12/humann3/ko.tsv # 1736 lines
grep -v '|' result12/humann3/ko.tsv > result12/humann3/ko_clean.tsv
wc -l result12/humann3/ko_clean.tsv

# 分层结果：功能和对应物种表(stratified)和功能组成表(unstratified)
humann_split_stratified_table \
--input result12/humann3/ko.tsv \
--output result12/humann3/ 
  
  # KO to level 1/2/3
  summarizeAbundance.py \
-i result12/humann3/ko_unstratified.tsv \
-m ${db}/EasyMicrobiome/kegg/KO1-4.txt \
-c 2,3,4 -s ',+,+,' -n raw \
-o result12/humann3/KEGG

## 2.5 GraPhlAn图

# metaphlan2 to graphlan
conda activate humann2
export2graphlan.py --skip_rows 1,2 -i result12/metaphlan4/taxonomy.tsv \
...
graphlan.py temp/merged_abundance.xml result12/metaphlan4/graphlan.pdf \
--external_legends 


## 2.6 LEfSe差异分析物种

*   输入文件：物种丰度表result12/metaphlan2/taxonomy.tsv
*   输入文件：样品分组信息 result12/metadata.txt
*   中间文件：整合后用于LefSe分析的文件 result12/metaphlan2/lefse.txt，这个文件可以提供给www\.ehbio.com/ImageGP 用于在线LefSE分析
*   LefSe结果输出：result12/metaphlan2/目录下lefse开头和feature开头的文件

前面演示数据仅有2个样本，无法进行差异比较。下面使用result1212目录中由12个样本生成的结果表进行演示

# 设置结果目录，自己的数据使用result12，演示用result1212
result12=result1212
# 如果没有，请下载演示数据
# wget -c http://www.imeta.science/db/EasyMetagenome/result1212.zip
# unzip result1212.zip

准备输入文件，修改样本品为组名(可手动修改)

# 预览输出数据
head -n3 $result12/metaphlan2/taxonomy.tsv
# 提取样本行，替换为每个样本一行，修改ID为SampleID
head -n1 $result12/metaphlan2/taxonomy.tsv|tr '\t' '\n'|sed '1 s/ID/SampleID/' >temp/sampleid
head -n3 temp/sampleid
# 提取SampleID对应的分组Group(假设为metadata.txt中第二列$2)，替换换行\n为制表符\t，再把行末制表符\t替换回换行
awk 'BEGIN{OFS=FS="\t"}NR==FNR{a[$1]=$2}NR>FNR{print a[$1]}' $result12/metadata.txt temp/sampleid|tr '\n' '\t'|sed 's/\t$/\n/' >groupid
cat groupid
# 合并分组和数据(替换表头)
cat groupid <(tail -n+2 $result12/metaphlan2/taxonomy.tsv) > $result12/metaphlan2/lefse.txt
head -n3 $result12/metaphlan2/lefse.txt
...
result12=result1212
# 格式转换为lefse内部格式
lefse-format_input.py $result12/metaphlan2/lefse.txt \
...
$result12/metaphlan2/lefse_cladogram.pdf --format pdf

# 绘制所有差异features柱状图
lefse-plot_res.py temp/input.res \
$result12/metaphlan2/lefse_res.pdf --format pdf
...
$result12/metaphlan2/lefse_Firmicutes.pdf

# 批量绘制所有差异features柱状图
lefse-plot_features.py -f diff \
--archive none --format pdf \
temp/input.in temp/input.res \
$result12/metaphlan2/lefse_
...
整合输出结果：result12/kraken2/taxonomy\_count.txt 物种丰度表
...
mkdir -p result12/kraken2
# 输出结果行数相同，但不一定顺序一致，要重新排序
tail -n+2 result12/metadata.txt|cut -f1|rush -j 1 \
'tail -n+2 temp/kraken2/{1}.mpa | LC_ALL=C sort | cut -f 2 | sed "1 s/^/{1}\n/" > temp/kraken2/{1}_count '
# 提取第一样本品行名为表行名
header=`tail -n 1 result12/metadata.txt | cut -f 1`
echo $header
...
paste temp/kraken2/*count > result12/kraken2/tax_count.mpa
# 检查表格及统计
csvtk -t stat result12/kraken2/tax_count.mpa
head -n 5 result12/kraken2/tax_count.mpa
...
# 提取第一样本品行名为表行名
h=`tail -n1 result12/metadata.txt|cut -f1`
tail -n+2 temp/bracken/${h}|sort|cut -f1 | \
sed "1 s/^/Taxonomy\n/" > temp/bracken/0header.count
# 检查文件数，为n+1
ls temp/bracken/*count | wc
# paste合并样本为表格，并删除非零行
paste temp/bracken/*count > result12/kraken2/bracken.${tax}.txt
# 统计行列，默认去除表头
csvtk -t stat result12/kraken2/bracken.${tax}.txt
# 按频率过滤，-r可标准化，-e过滤(microbiome_helper)
Rscript ${db}/EasyMicrobiome/script/filter_feature_table.R \
-i result12/kraken2/bracken.${tax}.txt \
-p 0.01 \
-o result12/kraken2/bracken.${tax}.0.01
csvtk -t stat result12/kraken2/bracken.${tax}.0.01

个性化结果筛选

# 门水平去除脊索动物(人)
grep 'Chordata' result12/kraken2/bracken.P.0.01
grep -v 'Chordata' result12/kraken2/bracken.P.0.01 > result12/kraken2/bracken.P.0.01-H

# 按物种名手动去除宿主污染，以人为例(需按种水平计算相关结果)
# 种水平去除人类P:Chordata,S:Homo sapiens
grep 'Homo sapiens' result12/kraken2/bracken.S.0.01
grep -v 'Homo sapiens' result12/kraken2/bracken.S.0.01 > result12/kraken2/bracken.S.0.01-H
...
megahit -t 3 \
-1 `tail -n+2 result12/metadata.txt|cut -f1|sed 's/^/temp\/qc\//;s/$/_1.fastq/'|tr '\n' ','|sed 's/,$//'` \
-2 `tail -n+2 result12/metadata.txt|cut -f1|sed 's/^/temp\/qc\//;s/$/_2.fastq/'|tr '\n' ','|sed 's/,$//'` \
-o temp/megahit 
...
# 备份重要结果
mkdir -p result12/megahit/
  ln -f temp/megahit/final.contigs.fa result12/megahit/
  # 删除临时文件
...
`tail -n+2 result12/metadata.txt|cut -f1|sed 's/^/temp\/qc\//;s/$/_1.fastq/'|sed 's/^/-1 /'| tr '\n' ' '` \
`tail -n+2 result12/metadata.txt|cut -f1|sed 's/^/temp\/qc\//;s/$/_2.fastq/'|sed 's/^/-2 /'| tr '\n' ' '` \
-o temp/metaspades
...
# 备份重要结果
mkdir -p result12/metaspades/
  ln -f temp/metaspades/contigs.fasta result12/metaspades/
  # 删除临时文件
...
quast.py result12/megahit/final.contigs.fa \
-o result12/megahit/quast -t 2

# (可选) megahit和metaspades比较
quast.py --label "megahit,metapasdes" \
result12/megahit/final.contigs.fa \
result12/metaspades/contigs.fasta \
-o result12/quast

# (可选)metaquast评估，更全面，但需下载相关数据库，受网速影响可能时间很长(经常失败)
# metaquast based on silva, and top 50 species genome, 18min
time metaquast.py result12/megahit/final.contigs.fa \
-o result12/megahit/metaquast

## 3.2 基因预测、去冗余和定量Gene prediction, cluster & quantitfy

### metaProdigal基因预测Gene prediction

# 输入文件：组装的序列 result12/megahit/final.contigs.fa
...
prodigal -i result12/megahit/final.contigs.fa \
...
# 输出文件：去冗余后的基因和蛋白序列：result12/NR/nucleotide.fa, result12/NR/protein.fa

mkdir -p result12/NR
# aS覆盖度，c相似度，G局部比对，g最优解，T多线程，M内存0不限制
# 2万基因2m，2千万需要2000h，多线程可加速
cd-hit-est -i temp/prodigal/gene.fa \
-o result12/NR/nucleotide.fa \
-aS 0.9 -c 0.95 -G 0 -g 0 -T 0 -M 0
# 统计非冗余基因数量，单次拼接结果数量下降不大，多批拼接冗余度高
grep -c '>' result12/NR/nucleotide.fa
# 翻译核酸为对应蛋白序列, --trim去除结尾的*
seqkit translate --trim result12/NR/nucleotide.fa \
> result12/NR/protein.fa 
# 两批数据去冗余使用cd-hit-est-2d加速，见附录

### salmon基因定量quantitfy

# 输入文件：去冗余后的基因序列：result12/NR/nucleotide.fa
# 输出文件：Salmon定量：result12/salmon/gene.count, gene.TPM

mkdir -p temp/salmon
salmon -v # 1.8.0

# 建索引, -t序列, -i 索引，10s
salmon index -t result12/NR/nucleotide.fa \
...
# 合并
mkdir -p result12/salmon
salmon quantmerge --quants temp/salmon/*.quant \
-o result12/salmon/gene.TPM
salmon quantmerge --quants temp/salmon/*.quant \
--column NumReads -o result12/salmon/gene.count
sed -i '1 s/.quant//g' result12/salmon/gene.*
  
  # 预览结果表格
  head -n3 result12/salmon/gene.*
  
  ## 3.3 功能基因注释Functional gene annotation
  
  # 输入数据：上一步预测的蛋白序列 result12/NR/protein.fa
  # 中间结果：temp/eggnog/protein.emapper.seed_orthologs
  #           temp/eggnog/output.emapper.annotations
  #           temp/eggnog/output
  
  # COG定量表：result12/eggnog/cogtab.count
  #            result12/eggnog/cogtab.count.spf (用于STAMP)
  
# KO定量表：result12/eggnog/kotab.count
#           result12/eggnog/kotab.count.spf  (用于STAMP)

# CAZy碳水化合物注释和定量：result12/dbcan2/cazytab.count
#                           result12/dbcan2/cazytab.count.spf (用于STAMP)

# 抗生素抗性：result12/resfam/resfam.count
#             result12/resfam/resfam.count.spf (用于STAMP)
...
-i result12/NR/protein.fa --cpu 3 -m diamond --override \
...
# 生成COG/KO/CAZy丰度汇总表
mkdir -p result12/eggnog
# 显示帮助
summarizeAbundance.py -h
# 汇总，7列COG_category按字母分隔，12列KEGG_ko和19列CAZy按逗号分隔，原始值累加
summarizeAbundance.py \
-i result12/salmon/gene.TPM \
-m temp/eggnog/output \
-c '7,12,19' -s '*+,+,' -n raw \
-o result12/eggnog/eggnog
sed -i 's#^ko:##' result12/eggnog/eggnog.KEGG_ko.raw.txt
sed -i '/^-/d' result12/eggnog/eggnog*
...
result12/eggnog/eggnog.KEGG_ko.raw.txt | \
sed 's/^\t/Unannotated\t/' \
> result12/eggnog/eggnog.KEGG_ko.TPM.spf
head -n 5 result12/eggnog/eggnog.KEGG_ko.TPM.spf
# KO to level 1/2/3
summarizeAbundance.py \
-i result12/eggnog/eggnog.KEGG_ko.raw.txt \
-m ${db}/EasyMicrobiome/kegg/KO1-4.txt \
-c 2,3,4 -s ',+,+,' -n raw \
-o result12/eggnog/KEGG

# CAZy
awk 'BEGIN{FS=OFS="\t"} NR==FNR{a[$1]=$2} NR>FNR{print a[$1],$0}' \
${db}/EasyMicrobiome/dbcan2/CAZy_description.txt result12/eggnog/eggnog.CAZy.raw.txt | \
sed 's/^\t/Unannotated\t/' > result12/eggnog/eggnog.CAZy.TPM.spf

# COG
awk 'BEGIN{FS=OFS="\t"} NR==FNR{a[$1]=$2"\t"$3} NR>FNR{print a[$1],$0}' \
/db/EasyMicrobiome/eggnog/COG.anno result12/eggnog/eggnog.COG_category.raw.txt > \
result12/eggnog/eggnog.COG_category.TPM.spf
...
-c 2 -s ',' -n raw \
-o result12/dbcan2/TPM
# 添加注释生成STAMP的spf格式，结合metadata.txt进行差异比较
awk 'BEGIN{FS=OFS="\t"} NR==FNR{a[$1]=$2} NR>FNR{print a[$1],$0}' \
${db}/EasyMicrobiome/dbcan2/CAZy_description.txt result12/dbcan2/TPM.CAZy.raw.txt | \
sed 's/^\t/Unannotated\t/' \
> result12/dbcan2/TPM.CAZy.raw.spf

head result12/dbcan2/TPM.CAZy.raw.spf
# 检查未注释数量，有则需要检查原因
grep 'Unannotated' result12/dbcan2/TPM.CAZy.raw.spf|wc -l
...
mkdir -p result12/card
# 启动rgi环境和记录版本
conda activate rgi
rgi main -v # 5.2.1

# 简化蛋白ID
cut -f 1 -d ' ' result12/NR/protein.fa > temp/protein.fa
# 这个错误忽略即可，不是报错，没有任何影响  grep: 写错误: 断开的管道
grep '>' result12/NR/protein.fa | head -n 3
grep '>' temp/protein.fa | head -n 3
# 蛋白层面注释ARG
rgi main -i temp/protein.fa -t protein \
-n 9 -a DIAMOND --include_loose --clean \
-o result12/card/protein
head -n3 result12/card/protein.txt

# 基因层面注释ARG
cut -f 1 -d ' ' result12/NR/nucleotide.fa > temp/nucleotide.fa
grep '>' temp/nucleotide.fa | head -n3
rgi main -i temp/nucleotide.fa -t contig \
-n 9 -a DIAMOND --include_loose --clean \
-o result12/card/nucleotide
head -n3 result12/card/nucleotide.txt

# 重叠群层面注释ARG
cut -f 1 -d ' ' result12/megahit/final.contigs.fa > temp/contigs.fa
grep '>' temp/contigs.fa | head -n3
rgi main -i temp/contigs.fa -t contig \
-n 9 -a DIAMOND --include_loose --clean \
-o result12/card/contigs
head result12/card/contigs.txt
...
temp/NRgene.taxid \
> result12/NR/nucleotide.tax
conda activate eggnog 
summarizeAbundance.py \
-i result12/salmon/gene.TPM \
-m result12/NR/nucleotide.tax \
-c '2,3,4,5,6,7,8,9' -s ',+,+,+,+,+,+,+,' -n raw \
-o result12/NR/tax
conda deactivate
wc -l result12/NR/tax*|sort -n
...
# 组装的重叠群文件：result12/megahit/final.contigs.fa
...
并行需要样本列表，请提前编写metadata.txt保存于result12中

# 快速读取文件生成样本ID列表再继续编写
ls temp/qc/ | grep _1 | cut -f 1 -d '_' | sort -u | sed '1 i SampleID' > result12/metadata.txt
# 预览
cat result12/metadata.txt
...
/bin/rm -rf temp/megahit_*
  tail -n+2 result12/metadata.txt|cut -f1|rush -j ${j} \
...
export GTDBTK_DATA_PATH="${db}/gtdb2.3"
# 10个基因组，24p，100min 152G内存; 6p,
gtdbtk classify_wf \
...
> result12/itol/tax.txt
head result12/itol/tax.txt
# 基因组评估信息
sed 's/,/\t/g;s/.fa//' temp/drep95/data_tables/Widb.csv|cut -f 1-7,11|sed '1 s/genome/ID/' \
> result12/itol/genome.txt
head result12/itol/genome.txt
# 整合注释文件
awk 'BEGIN{OFS=FS="\t"} NR==FNR{a[$1]=$0} NR>FNR{print $0,a[$1]}' result12/itol/genome.txt result12/itol/tax.txt|cut -f 1-8,10- > result12/itol/annotation.txt
head result12/itol/annotation.txt
...
*   C1\_genefamilies.tsv
*   整合后的输出：
*   result12/metaphlan2/taxonomy.tsv 物种丰度表
*   result12/metaphlan2/taxonomy.spf 物种丰度表（用于stamp分析）
*   result12/humann2/pathabundance\_relab\_unstratified.tsv 通路丰度表
*   result12/humann2/pathabundance\_relab\_stratified.tsv 通路物种组成丰度表
*   stratified(每个菌对此功能通路组成的贡献)和unstratified(功能组成)
...
mkdir -p result12/metaphlan2
# 合并、修正样本名、预览
merge_metaphlan_tables.py temp/humann2/*_metaphlan_bugs_list.tsv | \
sed 's/_metaphlan_bugs_list//g' > result12/metaphlan2/taxonomy.tsv
head -n5 result12/metaphlan2/taxonomy.tsv

#### 转换为stamp的spf格式

metaphlan_to_stamp.pl result12/metaphlan2/taxonomy.tsv \
> result12/metaphlan2/taxonomy.spf
head -n5 result12/metaphlan2/taxonomy.spf
# 下载metadata.txt和taxonomy.spf使用stamp分析
# 网络分析见附录 metaphlan2-共有或特有物种网络图
...
--in result12/metaphlan2/taxonomy.tsv \
--out result12/metaphlan2/heatmap.pdf \
...
注意看屏幕输出`# Gene table created: result12/humann2/pathabundance.tsv`

mkdir -p result12/humann2
humann2_join_tables --input temp/humann2 \
--file_name pathabundance \
--output result12/humann2/pathabundance.tsv
# 样本名调整：删除列名多余信息
head result12/humann2/pathabundance.tsv
sed -i 's/_Abundance//g' result12/humann2/pathabundance.tsv
# 预览和统计
head result12/humann2/pathabundance.tsv
csvtk -t stat result12/humann2/pathabundance.tsv

标准化为相对丰度relab(1)或百万比cpm(1,000,000)

humann2_renorm_table \
--input result12/humann2/pathabundance.tsv \
--units relab \
--output result12/humann2/pathabundance_relab.tsv
head -n5 result12/humann2/pathabundance_relab.tsv

分层结果：功能和对应物种表(stratified)和功能组成表(unstratified)

humann2_split_stratified_table \
--input result12/humann2/pathabundance_relab.tsv \
--output result12/humann2/ 
  # 可以使用stamp进行统计分析
...
*   输入数据：通路丰度表格 result12/humann2/pathabundance.tsv
*   输入数据：实验设计信息 result12/metadata.txt
*   中间数据：包含分组信息的通路丰度表格文件 result12/humann2/pathabundance.pcl
*   输出结果：result12/humann2/associate.txt

在通路丰度中添加分组

## 提取样品列表
head -n1 result12/humann2/pathabundance.tsv | sed 's/# Pathway/SampleID/' | tr '\t' '\n' > temp/header
## 对应分组，本示例分组为第2列($2)，根据实际情况修改
awk 'BEGIN{FS=OFS="\t"}NR==FNR{a[$1]=$2}NR>FNR{print a[$1]}' result12/metadata.txt temp/header | tr '\n' '\t'|sed 's/\t$/\n/' > temp/group
# 合成样本、分组+数据
cat <(head -n1 result12/humann2/pathabundance.tsv) temp/group <(tail -n+2 result12/humann2/pathabundance.tsv) \
> result12/humann2/pathabundance.pcl
head -n5 result12/humann2/pathabundance.pcl
tail -n5 result12/humann2/pathabundance.pcl

组间比较，样本量少无差异，结果为4列的文件：通路名字，通路在各个分组的丰度，差异P-value，校正后的Q-value。
演示数据2样本无法统计，此处替换为HMP的结果演示统计和绘图(上传hmp\_pathabund.pcl，替换pathabundance.pcl为hmp\_pathabund.pcl)。

wget -c http://www.imeta.science/github/EasyMetagenome/result12/humann2/hmp_pathabund.pcl
/bin/cp -f hmp_pathabund.pcl result12/humann2/
  # 设置输入文件名
  pcl=result12/humann2/hmp_pathabund.pcl
# 统计表格行、列数量
csvtk -t stat ${pcl}
head -n3 ${pcl} | cut -f 1-5
# 按分组KW检验，注意第二列的分组列名
...
--focal-metadatum Group --focal-type categorical \
--last-metadatum Group --fdr 0.05 \
--output result12/humann2/associate.txt
wc -l result12/humann2/associate.txt
head -n5 result12/humann2/associate.txt
...
--output result12/humann2/barplot_${path}.pdf
...
--input temp/humann2/ \
--file_name ko \
--output result12/humann2/ko.tsv
sed -i '1s/_Abundance-RPKs//g' result12/humann2/ko.tsv
tail result12/humann2/ko.tsv
# 与pathabundance类似，可进行标准化renorm、分层stratified、柱状图barplot等操作

KO合并为高层次L2, L1通路代码

wc -l result12/humann2/ko.tsv # 3797 lines
grep -v '|' result12/humann2/ko.tsv > result12/humann2/ko_clean.tsv
wc -l result12/humann2/ko_clean.tsv

# 分层结果：功能和对应物种表(stratified)和功能组成表(unstratified)
humann2_split_stratified_table \
--input result12/humann2/ko.tsv \
--output result12/humann2/ 
  
  # KO to level 1/2/3, 也可切换至humann3或qiime2等Python3环境下运行
  conda activate base
summarizeAbundance.py \
-i result12/humann2/ko_unstratified.tsv \
-m ${db}/EasyMicrobiome/kegg/KO1-4.txt \
-c 2,3,4 -s ',+,+,' -n raw \
-o result12/humann2/KEGG
conda deactivate
...
# metaphlan2 to graphlan
export2graphlan.py --skip_rows 1,2 -i result12/metaphlan2/taxonomy.tsv \
--tree temp/merged_abundance.tree.txt \
--annotation temp/merged_abundance.annot.txt \
--most_abundant 1000 --abundance_threshold 20 --least_biomarkers 10 \
--annotations 3,4 --external_annotations 7
...
temp/merged_abundance.tree.txt  temp/merged_abundance.xml
# output PDF figure, annoat and legend
graphlan.py temp/merged_abundance.xml result12/metaphlan2/graphlan.pdf \
--external_legends 
...
result12/metaphlan2/taxonomy.spf > result12/metaphlan2/taxonomy_highabundance.tsv

awk 'BEGIN{OFS=FS="\t"}{if(FNR==1) {print "Tax\tGrpcombine";} else a[$1]=a[$1]==""?$2:a[$1]$2;}END{for(i in a) print i,a[i]}' \
result12/metaphlan2/taxonomy_highabundance.tsv > result12/metaphlan2/taxonomy_group.tsv

cut -f 2 result12/metaphlan2/taxonomy_group.tsv | tail -n +2 | sort -u >group
...
group_colorcode result12/metaphlan2/taxonomy_group.tsv > result12/metaphlan2/taxonomy_group2.tsv

awk 'BEGIN{OFS=FS="\t"}{if(FNR==1) {print "Tax",$1,$2,$3,$4, $5, $6, $7, $8 } else print "Tax_"FNR, $1,$2,$3,$4, $5,$6, $7, $8}' \
result12/metaphlan2/taxonomy.spf > result12/metaphlan2/taxonomy_anno.tsv
...
combine_mpa.py \
-i `tail -n+2 result12/metadata.txt|cut -f1|sed 's/^/temp\/kraken2\//;s/$/.mpa/'|tr '\n' ' '` \
...
# 拆分contigs，按1M条每个文件
n=10000
seqkit split result12/megahit/final.contigs.fa -s $n
# 生成拆分文件序列列表
ls result12/megahit/final.contigs.fa.split/final.contigs.part_*.fa|cut -f 2 -d '_'|cut -f 1 -d '.' \
> temp/split.list
# 9线程并行基因预测，此步只用单线程且读写强度不大
time parallel -j 9 \
"prodigal -i result12/megahit/final.contigs.fa.split/final.contigs.part_{}.fa \
      -d temp/gene{}.fa  \
...
数据库：<http://www.dantaslab.org/resfams>
  ...
  mkdir -p temp/resfam result12/resfam
# 比对至抗生素数据库 1m
time diamond blastp \
...
wc -l temp/resfam/gene_fam.list  result12/salmon/gene.count 
# 按列表累计丰度
summarizeAbundance.py \
-i result12/salmon/gene.TPM \
-m temp/resfam/gene_fam.list \
-c 2 -s ',' -n raw \
-o result12/resfam/TPM
# 结果中添加FAM注释，spf格式用于stamp分析
awk 'BEGIN{FS=OFS="\t"} NR==FNR{a[$1]=$4"\t"$3"\t"$2} NR>FNR{print a[$1],$0}' \
${db}/resfam/Resfams-proteins_class.tsv  result12/resfam/TPM.ResGeneID.raw.txt \
> result12/resfam/TPM.ResGeneID.raw.spf
...
ls -sh temp/bin_annotate/prokka_out/bin.1/
  ...
  # 修改metadata
  sed 's/-/i/;s/\./i/' result12/metadatab.txt > result12/metadata.txt
# 修改文件名
awk 'BEGIN{OFS=FS="\t"}{system("mv temp/antismash/"$1".fna temp/antismash/"$2".fna")ll }' <(paste result12/metadatab.txt result12/metadata.txt|tail -n+2)
